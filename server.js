...
async function ensureGameBank(target=GAME_BANK_TARGET){if(bankGenerating||!process.env.OPENAI_API_KEY||gameBank.length>=target)return;clearTimeout(bankRetryTimer);bankRetryTimer=null;bankGenerating=true;try{let rejected=0;while(gameBank.length<target){const known=allKnownRecords(),categories=allKnownCategories();let candidate;try{candidate=await generateGame(known.map(x=>x.clue),categories);}catch(error){if(String(error.message||error).includes('failed validation')&&++rejected<8){bankLastError=`Rejected invalid board ${rejected}; retrying immediately.`;continue;}throw error;}if(gameHasDuplicate(candidate,known)||gameHasCategoryRepeat(candidate,categories)){if(++rejected>=8)throw new Error('Too many generated games repeated a stored clue or category.');bankLastError=`Rejected repeated board ${rejected}; retrying immediately.`;continue;}rejected=0;gameBank.push(candidate);bankLastError=null;saveBankSync();for(const room of rooms.values())if(room.phase==='lobby'){room.generating=gameBank.length<target;room.bankReady=gameBank.length;room.message=`${gameBank.length} of ${target} games ready.`;emit(room);}}}catch(error){bankLastError=String(error.message||error).slice(0,1000);console.warn('Question bank generation paused:',bankLastError);bankRetryTimer=setTimeout(()=>ensureGameBank(target),60000);bankRetryTimer.unref();}finally{bankGenerating=false;}}
