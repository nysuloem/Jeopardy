...
function safeGame(room){return {rounds:room.game.rounds.map((round,ri)=>({title:round.title,categories:round.categories.map((cat,ci)=>({name:cat.name,clues:cat.clues.map((q,ri2)=>({clue:room.selected?.category===ci&&room.selected?.row===ri2&&room.round===ri?q.clue:null,response:null}))}))})),final:{category:room.game.final.category,clue:['final_clue','final_answer','final_judging','final_reveal','final_results'].includes(room.phase)?room.game.final.clue:null,response:room.phase==='final_results'?room.game.final.response:null}};}
...
module.exports={app,server,io,rooms,makeRoom,publicRoom,introLine,values,dailyDouble,finishClue,phraseCorrect,prepareFinalReveal,advanceFinalReveal,dispose};
