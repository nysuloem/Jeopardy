...
function validateGame(game){
  return !!(game?.rounds?.length===2&&game.rounds.every((r,ri)=>r.categories?.length===6&&r.categories.every(c=>c.name&&c.clues?.length===5&&c.clues.every(q=>clueLengthValid(q.clue)&&q.response)&&categoryRuleValid(c))&&r.dailyDoubles?.length===(ri?2:1))&&game.final?.category&&clueLengthValid(game.final?.clue)&&game.final?.response);
}
...
async function generateFinalClue(avoid=[]){
  ...
  if(!item.category||!clueLengthValid(item.clue)||!item.response)throw new Error('Generated Final Jeopardy clue failed validation.');return item;
}
...
module.exports={FALLBACK_GAME,EMERGENCY_GAME,MAX_CLUE_CHARS,MAX_CLUE_WORDS,normalize,clueFingerprint,responseText,gameClueRecords,gameHasDuplicate,locallyCorrect,categoryRuleValid,beforeAfterValid,clueLengthValid,validateGame,generateGame,generateFinalClue,judge};
