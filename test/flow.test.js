...
const {server,io,rooms,makeRoom,publicRoom,dispose}=require('../server');
let url;
...
test('host, signed contestant, clue, buzz and scoring flow work together',async t=>{
  ...
  const joined=await player.emitWithAck('joinRoom',{code:room.code,name:'Jason',occupation:'biology professor',location:'London, Ontario',typedSignature:'Jason'});
  assert.equal(joined.ok,true);assert.equal(room.players[0].typedSignature,'Jason');
  assert.equal((await host.emitWithAck('startGame',{code:room.code})).ok,true);assert.equal(room.phase,'intro');
  host.emit('introFinished',{code:room.code});await pause(10);assert.equal(room.phase,'board');
  host.emit('selectClue',{code:room.code,category:0,row:0});await pause(10);assert.equal(room.phase,'clue');
